...
class AlreadyPaid : Fragment() {
    ...
    private lateinit var alreadyPaidModel: ArrayList<AlreadyPaidModel>
    private lateinit var alreadyPaidAdapter: AlreadyPaidAdapter
    ...
    override fun onCreateView(
        ...
        recyclerView = view.findViewById(R.id.rvAlreadyPaid)

        recyclerView.layoutManager = LinearLayoutManager(context)

        alreadyPaidModel = ArrayList()
                                ...
                                databaseReference.child(key1).child(key2).addValueEventListener(object :
                                    ...
                                    override fun onDataChange(snapshot: DataSnapshot) {
                                        ...
                                        for (dataSnapshot in snapshot.children) {
                                            ...
                                            if (kunci != "Info") {
                                                if (statusOrder.toString() == "Diproses") {
                                                    if (cc.toInt() > 1) {
                                                        alreadyPaidModel.add(AlreadyPaidModel(email,
                                                            count,
                                                            image,
                                                            orderId,
                                                            title,
                                        ...
                                        alreadyPaidAdapter = AlreadyPaidAdapter(alreadyPaidModel)
                                        alreadyPaidAdapter.notifyDataSetChanged()
                                        recyclerView.setHasFixedSize(true)
                                        recyclerView.adapter = alreadyPaidAdapter
